---
title: "Lupaire"
type: "Fiélon"
subtype: "Métamorphe"
size: "M"
alignment: "Neutre Mauvais"
challenge: "5"
customHP: "38"
abilityScores:
  for: 19
  dex: 14
  con: 18
  int: 10
  sag: 12
  cha: 15
savingThrows:
  - "dex"
  - "con"
  - "sag"
  - "cha"
ac:
  armorType: "custom"
  value: "15 (armure naturelle)"
skills:
  - name: "athletisme"
  - name: "discretion"
  - name: "intimidation"
  - name: "perception"
  - name: "supercherie"
movement:
  walk: 9
senses:
  darkvision: 36
conditionImmunities:
  - "charme"
  - "empoisonne"
  - "epuise"
damageTypeResistances:
  - "acide"
  - "foudre"
  - "c-p-t-non-argent"
damageTypeImmunities:
  - "feu"
  - "poison"
languages:
  - "commun"
  - "céleste"
  - "infernal"
telepathy: 30

---
## Lupaire
&nbsp;
[![Lupaire](https://www.douaratil.fr/illustrations/fielon/lupaire300.jpeg)](https://www.douaratil.fr/illustrations/fielon/lupaire.jpg)  
**<v-icon>mdi-head-sync</v-icon> Perte de santé mentale (SAM)**: DD 21, réussite 5, échec 3d6+2.  
**<v-icon>mdi-skull-crossbones</v-icon> Seuil de critique**: 18      
**<v-icon>mdi-shield</v-icon>Valeur offensive globale**: 17   
## <v-icon>mdi-weight-lifter</v-icon> Capacités
_**Métamorphe**_. Un lupaire peut utiliser une action pour se métamorphoser en humain, en canidé, ou pour prendre sa forme véritable, qui est celle d'un hybride entre un canidé et un humanoïde. Ses statistiques sont les même pour toutes ses formes, sauf en ce qui concerne son attaque de _griffe méphitique_. Tout équipement qu'il porte n'est pas transformé. Quand un lupaire est détruit, il reprend sa forme véritable avant de tomber en poussière.

_**Macule avilissante (1/jour)**_. Un lupaire peut sécréter une dégoûtante substance d'un jaune blanchâtre, visqueuse comme du goudron, afin de marquer sa nourriture et son territoire. Par une action bonus, un bouda marque un unique emplacement, objet ou créature sans défense qui se trouve dans un rayon de 1,50 mètre. Toute créature vivante qui se trouve dans un rayon de 9 mètres de la macule au début de son tour doit réussir un jet de sauvegarde de Constitution DD 15 contre le poison ou se trouver _empoisonnée_ pendant 1d6 rounds. Une créature qui réussit son jet de sauvegarde est immunisée à la macule avilissante de ce bouda pendant 24 heures. La puanteur que dégage une macule persiste pendant une semaine.

_**Incantation innée**_. La caractéristique d'incantation innée d'un lupaire est le Charisme (DD du jet de sauvegarde contre les sorts 13, +5 pour toucher avec les attaques de sort). Il peut lancer les sorts suivants sans utiliser de composante matérielle :
* **Constant** : [_détection de la magie_](/grimoire/detection-de-la-magie/), [_détection du mal et du bien_](/grimoire/detection-du-mal-et-du-bien/)
* **À volonté** : [_thaumaturgie_](/grimoire/thaumaturgie/)
* **3/jour** : [_repli expéditif_](/grimoire/repli-expeditif/), [_ténèbres_](/grimoire/tenebres/)
* **1/jour** : [_contagion_](/grimoire/contagion/)

## Actions
_**Attaques multiples**_. Un lupaire porte une attaque de _morsure_ et une attaque de _griffe méphitique_.

_**Morsure**_. _Attaque d'arme au corps à corps_ : +7 pour toucher, allonge 1,50 m, une cible.  
_Touché_ : 3d6+4 dégâts perforants plus 3d6 dégâts de poison.

_**Griffe méphitique**_. _Attaque d'arme au corps à corps_ : +7 pour toucher, allonge 1,50 m, une cible.  
_Touché_ : 3d6+4 dégâts tranchants, et la cible doit réussir un jet de sauvegarde de Constitution DD 15 ou se trouver _empoisonnée_ pour 1 round par le nuage de vermines qui grouille autour des bras du bouda.

_**Engloutissement vorace**_. Un lupaire dévore les organes d'un cadavre dans l'emplacement occupé par ce dernier. Le lupaire gagne une quantité de points d'action temporaires égale au niveau ou à l'ID de la créature morte pour une durée de 1 heure. Les organes qu'il a dévorés à l'aide de ce pouvoir n'existent plus.  
