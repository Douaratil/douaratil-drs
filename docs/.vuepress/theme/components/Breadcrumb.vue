<template>
  <div class="d-print-none">
    <v-breadcrumbs v-if="$route.path != '/'" :items="bread">
      <template v-slot:item="{ item }">
        <v-breadcrumbs-item :to="item.to" :exact="true" :disabled="item.disabled">
          ...
          <img src="/dragon_rouge.svg" style="width:20px;height:20px;" />
          </template>
          <template v-else>
            {{ item.text }}
          </template>
    ...
    <v-row v-else justify="center"><img class="d-block home-logo" src="/dragon_rouge.svg" style="width:120px;height:120px;"/></v-row>
  </div>
</template>
